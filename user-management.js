...
const EmployeeAuth = (() => {
    ...
    // ── قراءة/تهيئة قائمة المستخدمين من db.employees و db._settings ──────────
    function _ensureStore() {
        if (typeof db !== 'undefined') {
            if (!Array.isArray(db.employees)) {
                db.employees = Array.isArray(db._settings && db._settings.employees) ? db._settings.employees : [];
            }
            if (!db._settings) db._settings = {};
            db._settings.employees = db.employees;
            return db.employees;
        }
        return [];
    }
    ...
    // ── حفظ المستخدمين محلياً (localStorage + IndexedDB) والسحابياً (CloudSync) ──
    function _persistSettingsOnly() {
        ...
        } catch (e) { console.warn('[EmployeeAuth] failed to persist settings to localStorage', e); }

        // حفظ في IndexedDB
        if (typeof StorageEngine !== 'undefined' && typeof StorageEngine.save === 'function') {
            StorageEngine.save('employees', store).catch(e => console.warn('[EmployeeAuth] StorageEngine save employees failed', e));
        }

        // رفع وتزامن مع Firestore السحابي
        if (typeof CloudSync !== 'undefined' && CloudSync.isReady && CloudSync.isReady()) {
            try {
                CloudSync.syncTableNow('employees').catch(() => {});
                CloudSync.onLocalSave('employees');
            } catch (e) {}
        }

        // تحديث الواجهات المعروضة فوراً
        if (typeof renderSplashSecretaryQuickSelect === 'function') {
            try { renderSplashSecretaryQuickSelect(); } catch (e) {}
        }
        if (typeof renderEmployeeManagementList === 'function') {
            try { renderEmployeeManagementList(); } catch (e) {}
        }
    }
