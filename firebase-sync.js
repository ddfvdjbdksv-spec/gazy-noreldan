...
const CloudSync = (() => {
    ...
    const UPLOAD_COL_MAP = {
        ...
        employees: { id: 'id', name: 'nm', password: 'pw', active: 'ia', createdAt: 'ca' },
    ...
    const TABLE_UI_REFRESH_MAP = {
        ...
        employees: [
            () => { if (typeof window.renderEmployeeManagementList === 'function') window.renderEmployeeManagementList(); },
            () => { if (typeof window.renderSplashSecretaryQuickSelect === 'function') window.renderSplashSecretaryQuickSelect(); }
        ]
    };
    ...
    function attachSettingsListener() {
        settingsDocRef().onSnapshot(doc => {
            ...
            applyingRemote = true;
            // حماية قائمة المستخدمين من الضياع لو المستند السحابي خالٍ منها
            const localEmps = Array.isArray(db.employees) && db.employees.length ? db.employees : (Array.isArray(db._settings?.employees) ? db._settings.employees : []);
            db._settings = { ...db._settings, ...data };
            if (!Array.isArray(db._settings.employees) || db._settings.employees.length === 0) {
                db._settings.employees = localEmps;
            }
            db.employees = db._settings.employees;
            if (typeof StorageEngine !== 'undefined' && StorageEngine.save) {
                StorageEngine.save('employees', db.employees).catch(() => {});
            }
            applyingRemote = false;
            ...
            try { if (typeof window.renderSplashSecretaryQuickSelect === 'function') window.renderSplashSecretaryQuickSelect(); } catch (e) { }
            try { if (typeof window.renderEmployeeManagementList === 'function') window.renderEmployeeManagementList(); } catch (e) { }
            queueTableUIRefresh(null);
            updateLastSyncTime();
